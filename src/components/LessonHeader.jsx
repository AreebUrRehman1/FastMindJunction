...
export function LessonHeader({ darkModeControl, darkMode, mobile, navigate }) {

  const componentName = localStorage.getItem('component');

  return (
    <div className={`${darkMode ? "dark" : ""} fixed md:top-0 not-md:bottom-0 right-0 left-0 z-20 dark:bg-slate-900/80 border-slate-800 backdrop-blur-sm shadow-xl dark:border-b dark:border-gray-700`}>
      ...
      <div className="flex justify-between items-center max-w-7xl mx-auto px-4 py-3 md:py-2">
        ...
        {mobile ? (
          <div className={`flex flex-column items-center gap-x-3 text-blue-600 font-bold`} onClick={() => { navigate(`/learn/${componentName}`) }}>
